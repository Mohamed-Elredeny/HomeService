...
<?php if($isadmin == true){ ?>
    ...
    <br>
    <center><h4> <?php echo  "الطلبات قيد الإنتظار "  ?></h4></center>

<?php $counter=0; ?>
<?php  if(@count(@$MaintainRequests) > 0){ ?>
    <?php foreach ($MaintainRequests as $m){ ?>
        <?php if( SelectWithThree('userRequestIds','requestMaintainDepartment',$m['userId'],$m['requestId']) == 0 ){ ?>
...
<?php if($counter == 0){
    echo "<br><br><center><h3>لا توجد طلبات قيد الانتظار</h3> </center>";
    $counter=0;
} ?>

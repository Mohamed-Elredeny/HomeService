...
<?php if(@count($Clinicals) > 0){ ?>
    ...
    <?php  if(@count($MaintainRequests) > 0){ ?>
        <?php foreach ($MaintainRequests as $m){ ?>
            <?php if( SelectWithThree('userRequestIds','clinicTimeRequest',$m['userId'],$m['clinicRequestId'])== 1 ){ ?>
                <div class="card" style="height: 400px;width: 400px;display: inline-block;margin-left: 10px;margin-top: 20px;text-align: center;padding-top: 20px;padding-bottom: 70px">
                    طلب كشف
                    <div class="card-body">
                        <form action="OneClinicAcc.php?clinic_id=<?php echo $_GET['clinic_id'] ?>" method="post">
    ...
    <?php if($counter == 0){
        echo "<br><br><center><h3>لا توجد مواعيد حجز مقبولة بعد </h3> </center>";
        $counter=0;
    } } ?>
